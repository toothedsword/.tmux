...
hi Search		    ctermfg=196 ctermbg=none cterm=italic,underline
...
hi LineNr			ctermfg=27 guifg=red guibg=black  " 行号 
hi CursorLineNr		ctermfg=121 guibg=#cdcecd guifg=navyblue cterm=bold
hi Comment			ctermfg=116 guifg=red guibg=black cterm=italic " 注释 
hi Constant			ctermfg=42 guifg=#008900  " 变量
hi Identifier		ctermfg=81 guibg=#cdcecd guifg=black cterm=bold  "自定义函数
hi Statement		guifg=royalblue4 ctermfg=214
hi PreProc			ctermfg=51 guifg=#00ff00 cterm=bold
hi Type				ctermfg=43	guifg=#4a81b4 gui=NONE cterm=bold  " Exception
...
hi pythonDecorator          ctermfg=208 cterm=bold
hi pythonInclude            ctermfg=49  cterm=italic " Python imports, etc
hi pythonInstances          ctermfg=126  cterm=underline  
hi pythonStatement          ctermfg=84 cterm=bold " as, def
hi pythonConditional        ctermfg=214 cterm=bold " if, else
hi pythonRepeat	            ctermfg=202 cterm=underline  " for
hi pythonOperator           ctermfg=209  " in
hi pythonException          ctermfg=179  " try exception raise
...
hi pythonString             ctermfg=209 cterm=none " strings 33
hi pythonQuotes             ctermfg=220 " 引号
hi docstring                ctermfg=19 cterm=underline
hi pythonNumber guifg=#85B2FE gui=NONE  ctermfg=202
...
hi pythonFunc guifg=#A191F5 gui=NONE  ctermfg=81 cterm=italic 
hi pythonFuncDef guifg=#85CCFE gui=NONE  ctermfg=81 cterm=italic 
hi pythonFuncName guifg=#85B2FE gui=NONE  ctermfg=81 cterm=italic 
hi pythonFuncParams guifg=Red gui=NONE  ctermfg=81 cterm=italic 
hi pythonFunction guifg=DeepSkyBlue2 gui=NONE  ctermfg=81 cterm=underline,bold 
hi pythonCoding guifg=SlateGray3 gui=italic  ctermfg=1 cterm=italic 
hi pythonSelf guifg=#2FBBA6 gui=NONE  ctermfg=1 cterm=underline 
